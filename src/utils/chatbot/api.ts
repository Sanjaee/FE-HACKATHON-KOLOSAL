...
export async function fetchAgentTools(): Promise<any> {
  const res = await fetch("/api/agent?action=tools");
...
export async function sendAgentMessage(
  ...
  const res = await fetch("/api/agent?action=generate", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function detectObjects(
  ...
  const res = await fetch("/api/detect?action=segment", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      image,
...
export async function extractText(image: string, language = "auto"): Promise<any> {
  const res = await fetch("/api/ocr?action=form", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      image_data: image,
